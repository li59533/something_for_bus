 ...
 * @param    src:char* 
 * @param    delim:char 
 * @retval   char*: 返回新字符串起始地址
 */
char* CString_Split(char *src,char delim)
{
    char *p = src;
...
char* CString_nSplit(char *src,char delim,unsigned short len)
{
    char *p = src;
...
uint32_t CString_GetIp(char* pBuf)
{
    char* p_start = pBuf;
    uint32_t tmp = 0;
    ...
    if ((p_start = CString_Search(p_start,'.',1)) != NULL)
    {
        p_start++;
        tmp |= atoi(p_start)<<16;
    }
