...
void OS_Clock_AddMinutes(OSTime_t* start_dt,OSTime_t* new_dt,uint8_t minutes)
{
    *new_dt = *start_dt;
    new_dt->Minutes++;
    if (new_dt->Minutes >= 60)
    {
        new_dt->Minutes = 0;
        new_dt->Hour++;
        if (new_dt->Hour >= 24)
        {
            new_dt->Hour = 0;
            new_dt->Day++;
            if (new_dt->Day >= month_length(IsLeapYear(new_dt->Year), new_dt->Month))
            {
                new_dt->Day = 1;
                new_dt->Month++;
                if (new_dt->Month >= 13)
